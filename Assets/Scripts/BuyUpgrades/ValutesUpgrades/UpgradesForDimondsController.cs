...
public class UpgradesForDimondsController : MonoBehaviour
{
    ...
    public void BuyDimondUpgrades(BuyButtons BuyButton)
    {
        if (BuyButton.TypesOfDinondsUpgrades == TypesOfDinondsUpgrades.UpgradeForMoreCoins)
        {
            ...
            {
                if (Upgrades.Valute.IFruitDimonds.BasicValue >= (int)BuyButton.price[BuyButton.CurrentLevel])
                {
                    Upgrades.BuyUpgradeWithoutMultiForAnyValue(BuyButton,Upgrades.Valute.FruitDimonds.Valute,BuyButton.Values,(int)BuyButton.price[BuyButton.CurrentLevel]);
        ...
        if (BuyButton.TypesOfDinondsUpgrades == TypesOfDinondsUpgrades.UpgradeForMoreDimonds)
        {
            ...
            { 
                ...
                {
                    Upgrades.BuyUpgradeWithoutMultiForAnyValue(BuyButton,Upgrades.Valute.FruitDimonds.Valute,BuyButton.Values,(int)BuyButton.price[BuyButton.CurrentLevel]);
                    Upgrades.Valute.FruitDimonds.AddPercentForUpgrade();   
                }
            }   
        }
